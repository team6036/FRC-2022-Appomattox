...
public class Spark extends CANSparkMax implements Controller {
  ...
  static class SparkController extends ProfiledControllerBase<Spark> {
    ...
    protected void updateGains(
      ...
      if (isFirstInitialization) {
        mPidController.setSmartMotionAccelStrategy(
            SparkMaxPIDController.AccelStrategy.kSCurve, slot);
      }
    }
    ...
    protected boolean setReference(
      ...
      return mPidController.setReference(
              reference,
              controllerType,
              slot,
  ...
  protected static final Map<ControllerOutput.Mode, ControlType> kModeToController =
      Map.ofEntries(
  ...
  public void setOutputRange(double minimumOutput, double maximumOutput) {
    SparkMaxPIDController controller = getController();
    for (int slotIndex = 0; slotIndex <= 3; slotIndex++)
      controller.setOutputRange(minimumOutput, maximumOutput, slotIndex);
  }
  ...
  /**
   * As of 2/17/20 Spark controllers {@link CANSparkMax#getPIDController()} returns a new object.
   * So, it causes issues in loops and should be cached instead.
   *
   * @return Cached {@link SparkMaxPIDController} instance.
   */
