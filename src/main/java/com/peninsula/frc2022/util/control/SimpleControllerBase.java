...
public abstract class SimpleControllerBase<TController extends Controller> {
  ...
  private static final int kDefaultSlot = 0,
      kProfiledPositionSlot = 1,
      kProfiledVelocitySlot = 2,
      kVelocitySlot = 3;
  private static Map<ControllerOutput.Mode, Integer> sModeToSlot =
      Map.of(
  ...
  public boolean setOutput(ControllerOutput output) {
    ...
    boolean
        isProfiled =
            mode == ControllerOutput.Mode.PROFILED_POSITION
                || mode == ControllerOutput.Mode.PROFILED_VELOCITY,
        requiresGains =
            isProfiled
                || mode == ControllerOutput.Mode.POSITION
    ...
    if (!areGainsEqual
        || slot != mLastSlot
        || mode != mLastMode
        || reference != mLastReference
        || arbitraryPercentOutput != mLastArbitraryPercentOutput) {
      if (setReference(mode, slot, reference * 0.1, arbitraryPercentOutput)) {
        mLastSlot = slot;
        mLastMode = mode;
          ...
          if (isProfiled) {
            ...
            mLastGains.put(
                ...
                new ProfiledGains(
                    ...
                    gains.iZone,
                    gains.iMax,
                    profiledGains.acceleration,
                    profiledGains.velocity,
                    profiledGains.allowableError,
                    profiledGains.minimumOutputVelocity));
          } else {
            mLastGains.put(
                slot, new Gains(gains.p, gains.i, gains.d, gains.f, gains.iZone, gains.iMax));
          }
        ...
        Log.error(
            "controller", String.format("Error updating output on controller: %s", getName()));
      }
    }
    return false;
  }
  ...
  protected void updateGainsIfChanged(Gains gains, int slot) {
    if (gains != null) {
      ...
      if (isFirstInitialization) { // Empty gains for default value instead of null
        mLastGains.put(
            slot,
            (slot == kProfiledPositionSlot || slot == kProfiledVelocitySlot)
                ? new ProfiledGains()
                : new Gains());
      }
  ...
  protected abstract boolean setReference(
   ...
   * @param statusFrameMs Period for received information from the controller. Needs to be similar
   *     to closed loop control loop period, since it contains encoder information
   */
