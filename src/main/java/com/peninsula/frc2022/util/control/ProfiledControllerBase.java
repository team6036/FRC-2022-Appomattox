...
public abstract class ProfiledControllerBase<TController extends Controller>
    extends SimpleControllerBase<TController> {
  ...
  protected void updateGains(
    ...
    if (newGains instanceof ProfiledGains) {
      ProfiledGains lastProfiledGains = (ProfiledGains) lastGains,
          newProfiledGains = (ProfiledGains) newGains;
      ...
      if (Double.compare(
              lastProfiledGains.minimumOutputVelocity, newProfiledGains.minimumOutputVelocity)
          != 0) {
